...
def benchmark_case(field_name, n, k, seed=42):
    ...
    match = "OK" if our_weight == sage_weight else "FAIL"
...
for field_name, n, k in test_cases:
    try:
        ...
        if not result['correct']:
            all_correct = False
            print(f"WARNING: НЕВЕРНЫЙ РЕЗУЛЬТАТ для {field_name}[{n},{k}]!")

    except Exception as e:
        print(f"FAIL ОШИБКА для {field_name}[{n},{k}]: {e}")
        all_correct = False
...
if all_correct:
    ...
    print("FAIL Обнаружены ошибки в результатах!")
...
for result in results:
    ...
    if result['speedup'] >= 1.0:
        speedup = f"{result['speedup']:.1f}x OK"
    else:
        speedup = f"{1/result['speedup']:.1f}x FAIL"
